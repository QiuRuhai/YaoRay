...
std::string_view RenderSamplerName(RenderSamplerKind sampler) {
    switch (sampler) {
        ...
        case RenderSamplerKind::OwenSobol: return "sobol";
        case RenderSamplerKind::ZSobol: return "zsobol";
    }
    return "unknown";
}
...
std::optional<RenderSamplerKind> ParseRenderSamplerName(std::string_view name) {
    ...
    if (name == "sobol" || name == "paddedsobol") return RenderSamplerKind::OwenSobol;
    if (name == "zsobol") return RenderSamplerKind::ZSobol;
    return std::nullopt;
}
