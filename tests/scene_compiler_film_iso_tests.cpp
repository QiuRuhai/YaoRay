...
YR_TEST(scene_compiler_film_iso_100_is_zero_stops) {
    ...
    YR_EXPECT_NEAR(result.scene->film.exposure, 0.0f, 1e-6f);
}
...
YR_TEST(scene_compiler_film_iso_200_is_one_stop) {
    ...
    YR_EXPECT_TRUE(result.scene.has_value());
    YR_EXPECT_NEAR(result.scene->film.exposure, 1.0f, 1e-6f);
}
...
YR_TEST(scene_compiler_film_iso_500_matches_pavilion_setting) {
    ...
    YR_EXPECT_NEAR(result.scene->film.exposure, expected, 1e-6f);
}
