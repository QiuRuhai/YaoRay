...
YR_TEST(material_evaluator_resolve_returns_base_material_for_untextured) {
    ...
    const yr::TriangleRef tri = yr::LocateTriangle(scene.Geometry(), 0);
    const yr::ResolvedMaterial sample = yr::EvaluateMaterialAtSurface(
        yr::MakeShadingSceneView(scene), tri, scene.materials[0],
        0.33f, 0.33f,
