...
// TODO(Task 11): Expand backend tests for table-geometry API.
...
YR_TEST(cpu_backend_prepare_builds_cpu_prepared_scene) {
    ...
    yr::BackendPrepareResult prepared = backend->Prepare(std::move(scene));
    ...
    YR_EXPECT_EQ(prepared.scene->SourceScene().width, 4);
    YR_EXPECT_EQ(prepared.scene->SourceScene().height, 3);
}
...
YR_TEST(cuda_prepared_scene_exposes_source_scene) {
    yr::RenderSceneIR scene = MakeBackendTriangleScene(2, 2);

    const yr::CudaPreparedScene prepared(std::move(scene));

    YR_EXPECT_EQ(prepared.Kind(), yr::RenderBackendKind::Cuda);
    YR_EXPECT_EQ(prepared.SourceScene().width, 2);
    YR_EXPECT_EQ(prepared.SourceScene().height, 2);
}
