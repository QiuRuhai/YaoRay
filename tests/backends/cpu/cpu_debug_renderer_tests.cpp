...
yr::CpuPreparedScene PrepareDebugScene(yr::RenderJob job) {
...
yr::RenderJob MakeDebugTriangleJob(int width = 5, int height = 5) {
    yr::RenderJob job;
...
YR_TEST(cpu_debug_renderer_traces_one_ray_per_pixel) {
    yr::RenderJob job = MakeDebugTriangleJob(4, 3);

    const yr::CpuDebugRenderResult result = yr::RenderCpuDebug(PrepareDebugScene(std::move(job)));
...
YR_TEST(cpu_debug_renderer_records_hits_and_misses) {
    yr::RenderJob job = MakeDebugTriangleJob(5, 5);
...
YR_TEST(cpu_debug_renderer_shades_environment_misses) {
    yr::RenderJob job = MakeDebugTriangleJob(2, 2);
    job.scene.vertices.clear();
    job.scene.indices.clear();
    job.scene.primitives.clear();
    job.scene.materials.clear();
    job.scene.environment.active = true;
    job.scene.environment.radiance = yr::Color3f{0.2f, 0.3f, 0.4f};
    job.scene.environment.strength = 2.0f;
