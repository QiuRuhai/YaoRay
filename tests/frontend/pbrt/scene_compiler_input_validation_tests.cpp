...
YR_TEST(scene_compiler_clamps_invalid_render_settings_with_warnings) {
    ...
    YR_EXPECT_EQ(result.settings.width, 1);
    YR_EXPECT_EQ(result.settings.height, 1);
    YR_EXPECT_EQ(result.settings.spp, 1);
    YR_EXPECT_EQ(result.settings.max_depth, 0);
...
YR_TEST(scene_compiler_clamps_huge_film_resolution_with_warnings) {
    ...
    YR_EXPECT_EQ(result.settings.width, 16384);
    YR_EXPECT_EQ(result.settings.height, 16384);
}
...
YR_TEST(scene_compiler_clamps_huge_numeric_settings_before_int_conversion) {
    ...
    YR_EXPECT_EQ(result.settings.spp, 1);
    YR_EXPECT_EQ(result.settings.max_depth, 5);
}
...
YR_TEST(scene_compiler_honors_ysamples_without_xsamples) {
    ...
    YR_EXPECT_EQ(result.settings.spp, 4);
}
