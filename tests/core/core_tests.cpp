...
YR_TEST(precomputed_slab_handles_parallel_boundary_and_negative_rays) {
    const yr::Bounds3f box{
        yr::Point3f{-1.0f, -1.0f, -1.0f}, yr::Point3f{1.0f, 1.0f, 1.0f}};
    const yr::Ray3f inside{
        yr::Point3f{0.0f, 0.0f, -3.0f}, yr::Vec3f{0.0f, 0.0f, 1.0f}};
    const yr::Ray3f boundary{
        yr::Point3f{1.0f, 0.0f, -3.0f}, yr::Vec3f{0.0f, 0.0f, 1.0f}};
    const yr::Ray3f outside{
        yr::Point3f{2.0f, 0.0f, -3.0f}, yr::Vec3f{0.0f, 0.0f, 1.0f}};
    const yr::Ray3f negative{
        yr::Point3f{0.0f, 0.0f, 3.0f}, yr::Vec3f{-0.0f, 0.0f, -1.0f}};
    float entry = 0.0f;

    YR_EXPECT_TRUE(yr::IntersectBounds(
        box, yr::PrecomputeRayBounds(inside), 0.001f, 100.0f, entry));
    YR_EXPECT_NEAR(entry, 2.0f, 1.0e-6f);
    YR_EXPECT_TRUE(yr::IntersectBounds(
        box, yr::PrecomputeRayBounds(boundary), 0.001f, 100.0f, entry));
    YR_EXPECT_TRUE(!yr::IntersectBounds(
        box, yr::PrecomputeRayBounds(outside), 0.001f, 100.0f, entry));
    YR_EXPECT_TRUE(yr::IntersectBounds(
        box, yr::PrecomputeRayBounds(negative), 0.001f, 100.0f, entry));
    YR_EXPECT_NEAR(entry, 2.0f, 1.0e-6f);
}
